...
| **Thinking and tool activity** | Matrix `tool_progress` defaults to **off** so tool streams do not permanently clutter the room timeline. Opt in with `display.platforms.matrix.tool_progress: all` or `new`. When enabled, progress normally accumulates into one editable message via `m.replace` rather than a new timeline line per tool. Every `m.replace` is still a room event — clients/bridges that do not aggregate replacements may still surface updates in history or notifications, which is why the default stays quiet. Thinking panes stay off by default. |
...
When the Matrix gateway is connected, Hermes can expose compact `matrix` / `matrix_admin` tools (core/admin capability split).
