"""Gateway integration tests for lossless Matrix introspection fields."""
...
class MatrixIntrospectionAdapter(BasePlatformAdapter):
    def __init__(self):
    ...
    async def start_thinking(
        ...
        room_id,
        task_id,
        initial_summary="Processing request...",
        model_label="",
        initial_content_md="",
    ):
        self.calls.append(("start_thinking", initial_summary, model_label, initial_content_md))
        return "$thinking"

    async def update_thinking(self, task_id, step_info, content_md="", model_label=None):
        self.calls.append(("update_thinking", step_info, content_md, model_label))

    async def finalize_thinking(self, task_id, final_summary="Task complete", collapse=True, model_label=None):
        self.calls.append(("finalize_thinking", final_summary, model_label))

    async def start_tool_activity(
        self,
        room_id,
        task_id,
        initial_summary="Tool activity",
        model_label="",
        initial_content_md="",
    ):
        self.calls.append(("start_tool_activity", initial_summary, model_label, initial_content_md))
        return "$tools"

    async def update_tool_activity(self, task_id, step_info, content_md="", model_label=None):
        self.calls.append(("update_tool_activity", step_info, content_md, model_label))

    async def finalize_tool_activity(self, task_id, final_summary="Tool activity complete", collapse=True, model_label=None):
        self.calls.append(("finalize_tool_activity", final_summary, model_label))
...
class FakeAgent:
    ...
    def run_conversation(self, message, conversation_history=None, task_id=None):
        if self.status_callback:
            self.status_callback("info", "🔄 Primary model failed — switching to fallback: gpt-5.4 via openai-codex")
        if self.thinking_callback:
            self.thinking_callback("Working through the request")
        if self.reasoning_callback:
            self.reasoning_callback("Detailed reasoning delta")
        if self.tool_progress_callback:
            self.tool_progress_callback("terminal", "pwd")
            self.tool_progress_callback("search_files", "turn_route")
...
@pytest.mark.asyncio
async def test_matrix_run_routes_status_reasoning_and_tools_to_buffered_fields(monkeypatch, tmp_path):
    ...
    result = await runner._run_agent(
        ...
        source=source,
        session_id="sess-1",
        session_key="agent:main:matrix:dm:!room:example.org",
    )
    ...
    tool_payloads = [c[-1] for c in adapter.calls if c[0] == "start_tool_activity"] + [
        c[2] for c in adapter.calls if c[0] == "update_tool_activity"
    ]
