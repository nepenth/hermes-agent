"""
Tests for the OpenAI-compatible HTTP server gateway adapter.
...
from gateway.platforms.http_server import (
    HTTPServerAdapter,
    ResponseStore,
    _CORS_HEADERS,
    check_http_server_requirements,
    cors_middleware,
)
...
class TestCheckRequirements:
    def test_returns_true_when_aiohttp_available(self):
        assert check_http_server_requirements() is True

    @patch("gateway.platforms.http_server.AIOHTTP_AVAILABLE", False)
    def test_returns_false_without_aiohttp(self):
        assert check_http_server_requirements() is False
...
class TestAdapterInit:
    def test_default_config(self):
        ...
        adapter = HTTPServerAdapter(config)
        assert adapter._host == "127.0.0.1"
        assert adapter._port == 8642
        assert adapter._api_key == ""
        assert adapter.platform == Platform.HTTP_SERVER
    ...
    def test_config_from_env(self, monkeypatch):
        monkeypatch.setenv("HTTP_SERVER_HOST", "10.0.0.1")
        monkeypatch.setenv("HTTP_SERVER_PORT", "7777")
        monkeypatch.setenv("HTTP_SERVER_KEY", "sk-env")
        config = PlatformConfig(enabled=True)
        adapter = HTTPServerAdapter(config)
...
def _make_adapter(api_key: str = "") -> HTTPServerAdapter:
...
def _create_app(adapter: HTTPServerAdapter) -> web.Application:
...
class TestConfigIntegration:
    def test_platform_enum_has_http_server(self):
        assert Platform.HTTP_SERVER.value == "http_server"

    def test_env_override_enables_http_server(self, monkeypatch):
        monkeypatch.setenv("HTTP_SERVER_ENABLED", "true")
        from gateway.config import load_gateway_config
        config = load_gateway_config()
        assert Platform.HTTP_SERVER in config.platforms
        assert config.platforms[Platform.HTTP_SERVER].enabled is True

    def test_env_override_with_key(self, monkeypatch):
        monkeypatch.setenv("HTTP_SERVER_KEY", "sk-mykey")
        from gateway.config import load_gateway_config
        config = load_gateway_config()
        assert Platform.HTTP_SERVER in config.platforms
        assert config.platforms[Platform.HTTP_SERVER].extra.get("key") == "sk-mykey"

    def test_env_override_port_and_host(self, monkeypatch):
        monkeypatch.setenv("HTTP_SERVER_ENABLED", "true")
        monkeypatch.setenv("HTTP_SERVER_PORT", "9999")
        monkeypatch.setenv("HTTP_SERVER_HOST", "0.0.0.0")
        from gateway.config import load_gateway_config
        config = load_gateway_config()
        assert config.platforms[Platform.HTTP_SERVER].extra.get("port") == 9999
        assert config.platforms[Platform.HTTP_SERVER].extra.get("host") == "0.0.0.0"

    def test_http_server_in_connected_platforms(self):
        config = GatewayConfig()
        config.platforms[Platform.HTTP_SERVER] = PlatformConfig(enabled=True)
        connected = config.get_connected_platforms()
        assert Platform.HTTP_SERVER in connected

    def test_http_server_not_in_connected_when_disabled(self):
        config = GatewayConfig()
        config.platforms[Platform.HTTP_SERVER] = PlatformConfig(enabled=False)
        connected = config.get_connected_platforms()
        assert Platform.HTTP_SERVER not in connected
