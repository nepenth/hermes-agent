...
class MatrixTestRunner(GatewayAuthorizationMixin, SimpleNamespace):
    """Use the current production adapter-resolution chain in integration tests."""


def test_profile_resolver_stays_fail_closed(monkeypatch):
    primary, secondary = object(), object()
    runner = MatrixTestRunner(
        adapters={Platform.MATRIX: primary},
        _profile_adapters={"secondary": {Platform.MATRIX: secondary}, "failed": {}},
    ...
    with patch("gateway.run._gateway_runner_ref", return_value=runner):
        for profile, expected in (("primary", primary), ("secondary", secondary),
            ...
            assert bool(error) is (expected is None)
