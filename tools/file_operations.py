...
class ShellFileOperations(FileOperations):
    ...
    def search(
        ...
        if "not_found" in check.stdout:
            return SearchResult(
                error=f"Path not found: {path}. Verify the path exists (use 'terminal' to check).", total_count=0
            )
    ...
    def _search_content(
        self, pattern: str, path: str, file_glob: str | None, limit: int, offset: int, output_mode: str, context: int
    ) -> SearchResult:
