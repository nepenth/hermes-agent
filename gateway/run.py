...
class GatewayRunner:
            ...
            try:
                ...
                if _hyg_cfg_path.exists():
                    ...
                    if isinstance(_comp_cfg, dict):
                        _hyg_threshold_pct = float(_comp_cfg.get("threshold", _hyg_threshold_pct))
                        _hyg_compression_enabled = str(_comp_cfg.get("enabled", True)).lower() in ("true", "1", "yes")
            except Exception:
                pass

            # Also check env overrides (same as run_agent.py)
            _hyg_threshold_pct = float(os.getenv("CONTEXT_COMPRESSION_THRESHOLD", str(_hyg_threshold_pct)))
                        ...
                        if _hyg_runtime.get("api_key"):
                            ...
                            if len(_hyg_msgs) >= 4:
                                ...
                                if _new_tokens >= _warn_token_threshold:
                                    logger.warning(
                                        "Session hygiene: still ~%s tokens after compression — suggesting /reset",
                                        f"{_new_tokens:,}",
                                    )
                                    if _hyg_adapter:
                                        try:
    ...
    async def _handle_compress_command(self, event: MessageEvent) -> str:
        ...
        try:
            ...
            return f"🗜️ Compressed: {original_count} → {new_count} messages\n~{approx_tokens:,} → ~{new_tokens:,} tokens"
        except Exception as e:
    ...
    async def _handle_reload_mcp_command(self, event: MessageEvent) -> str:
        ...
        loop = asyncio.get_event_loop()
        try:
            from tools.mcp_tool import _load_mcp_config, _lock, _servers, discover_mcp_tools, shutdown_mcp_servers
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        ...
        try:
            ...
            if systemd_run:
                subprocess.Popen(
                    [systemd_run, "--user", "--scope", "--unit=hermes-update", "--", "bash", "-c", update_cmd],
...
async def start_gateway(config: GatewayConfig | None = None, replace: bool = False) -> bool:
    """
